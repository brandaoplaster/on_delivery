defmodule OnDelivery.Orders.ItemTest do
  use ExUnit.Case
  ...
  describe "build/4" do
    test "when all params are valid, returns an item" do
      response = Item.build("Pizza 4 queijos", :pizza, "30.00", 1)
    ...
    test "when there is an invalid category, returns an error" do
      response = Item.build("Pizza 4 queijos", :uva, "30.00", 1)
    ...
    test "when there is an invalid price, returns an error" do
      response = Item.build("Pizza 4 queijos", :pizza, "sem", 1)
    ...
    test "when there is an invalid quantity, returns an error" do
      response = Item.build("Pizza 4 queijos", :pizza, "30.00", 0)
